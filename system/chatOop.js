...
async tinyurl(url) {
    ...
    async userName(id = this.senderID) {
        ...
        try {
            if (typeof this.api.getInfo === 'function') {
            ...
            this.log("Error using api.getInfo: " + err.message);
        }

        try {
            if (typeof this.api.getUserInfo === 'function') {
                const userInfo = await this.api.getUserInfo(id);
                if (userInfo && userInfo[id] && userInfo[id].name) {
                    return userInfo[id].name;
                }
            }
        } catch (err) {
            this.log("Error using api.getUserInfo: " + err.message);
        }
