...
let login = require("@eryxenx/fca");
if (typeof login !== "function" && typeof login.default === "function") {
    login = login.default;
}
...
let kokoro_config = JSON.parse(fs.readFileSync('./config.json', 'utf-8'));
...
app.use((req, res, next) => {
    res.setHeader('x-powered-by', 'Aminul AI');
    next();
});
